...
function queriesCacheDir() {
  return path.join(home(), 'cache', 'queries');
}
...
// Cache par hash du texte (sujets ET queries) — même format {model, vector}.
...
function readCache(hash) {
  return readCacheJSON(cacheFile(hash));
}
...
/**
 * embedQuery(text, embedder) -> Float32Array | null (embedder null).
 * Cache par sha256("query: "+text) sous cache/queries/ — même mécanique que
 * embedTopics, utilisé par cmdQuery et par le daemon `calepin serve`.
 */
export async function embedQuery(text, embedder) {
  if (!embedder) return null;
  const prefixed = `query: ${text}`;
  const hash = hashContent(prefixed);
  const file = path.join(queriesCacheDir(), `${hash}.json`);
  const cached = readCacheJSON(file);
  if (cached && cached.model === MODEL_ID) return Float32Array.from(cached.vector);

  const [vector] = await embedder.embed([prefixed]);
  fs.mkdirSync(queriesCacheDir(), { recursive: true });
  fs.writeFileSync(file, JSON.stringify({ model: MODEL_ID, vector: Array.from(vector) }));
  return vector;
}
